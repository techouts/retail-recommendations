...
#     # Build Query
#     # ------------------------
#     def build_query(l1=None, l2=None, l3=None, exclude_ids=None, limit=10):
#         must_clauses = []
...
#             else:
#                 must_clauses.append({"term": {"l2.keyword": l2[0] if isinstance(l2, list) else l2}})

#         if l3:
#             if isinstance(l3, list) and len(l3) > 1:
#                 must_clauses.append({"terms": {"l3.keyword": l3}})
#             else:
#                 must_clauses.append({"term": {"l3.keyword": l3[0] if isinstance(l3, list) else l3}})
...
#     # Step 1: Exact match
#     # ------------------------
#     if l3 or l2 or l1:  # run only if at least one filter
#         resp = es.search(index=alias, body=build_query(l1, l2, l3, limit=size))
#         hits = [hit["_source"] for hit in resp["hits"]["hits"]]
#         collected_products.extend(hits)

#     # ------------------------
#     # Step 2: Fallback to siblings within same l2
#     # ------------------------
#     if l2 and len(collected_products) < size:
#         used_siblings = True
...
#             aggs_query = {
#                 "size": 0,
#                 "query": {"term": {"l2.keyword": current_l2}},
#                 "aggs": {
#                     "unique_l3": {"terms": {"field": "l3.keyword", "size": 100}}
#                 }
#             }
...
#             exclude_ids = [p["product_id"] for p in collected_products]
#             for sibling_l3 in all_l3_values:
#                 if l3 and sibling_l3 in (l3 if isinstance(l3, list) else [l3]):
#                     continue
#                 remaining = size - len(collected_products)
#                 if remaining <= 0:
#                     break

#                 resp = es.search(index=alias, body=build_query(l1, current_l2, sibling_l3, exclude_ids, limit=remaining))
#                 hits = [hit["_source"] for hit in resp["hits"]["hits"]]
#                 collected_products.extend(hits)
#                 exclude_ids.extend([h["product_id"] for h in hits])

#     # ------------------------
#     # Step 3: Fallback to sibling l2 within same l1
#     # ------------------------
#     if l1 and len(collected_products) < size:
#         used_siblings = True
...
#         exclude_ids = [p["product_id"] for p in collected_products]
#         for sibling_l2 in all_l2_values:
#             if l2 and sibling_l2 in (l2 if isinstance(l2, list) else [l2]):
#                 continue
#             remaining = size - len(collected_products)
#             if remaining <= 0:
#                 break
...
#     for src in hits:
#         l1_val, l2_val, l3_val = src["l1"], src["l2"], src["l3"]
#         product = {
...
#             "trending_score": src["trending_score"]
#         }
#         category_dict[l1_val][l2_val][l3_val].append(product)
