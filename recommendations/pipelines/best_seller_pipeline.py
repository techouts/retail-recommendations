...
def run_bestseller_pipeline(settings: dict, time_window: dict, client: str):
    ...
    logger.info(f"Thresholds: { { 'sales': sales_t, 'revenue': revenue_t, 'stock': stock_t, 'return': return_t, 'rating': rating_t, 'final': final_t } }")
    ...
    # FIX 1: map 'rating' column to 'customer_rate'
    if "rating" in analytics_rating.columns:
        analytics_rating["customer_rate"] = pd.to_numeric(analytics_rating["rating"], errors="coerce").fillna(0)
    elif "customer_rate" not in analytics_rating.columns:
        analytics_rating["customer_rate"] = 0

    logger.info(f"analytics_rating columns: {analytics_rating.columns.tolist()}")
    logger.info(f"analytics_rating customer_rate sample: {analytics_rating['customer_rate'].head(5).tolist()}")
    ...
    # FIX 2: analytics_rating uses 'created_at', copy to 'timestamp' for time filter
    parse_datetime(analytics_rating, "created_at")
    analytics_rating["timestamp"] = analytics_rating["created_at"]
    ...
    logger.info(f"After time filter → fulfillment: {len(f)}, ratings: {len(a)}")
    logger.info(f"Sample customer_rate in ratings: {a['customer_rate'].head(5).tolist()}")
    ...
    logger.info(f"Aggregated ratings rows: {len(a)}")
    if not a.empty:
        logger.info(f"Avg rating sample: {a['avg_rating'].head(5).tolist()}")
    ...
    es_data = es_data.sort_values(["l3","final_score"], ascending=[True, False]).round(2)
